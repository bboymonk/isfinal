...
(function($) {
        ...
        init: function(options, swfUploadOptions) {
            ...
            return this.each(function() {
                ...
                if (flashInstalled) {
                    ...
                    $button.html('<a href="javascript:;" class="uploadify-button-text btn-uploadfile">' +
                            '<span class="l"></span><span class="c">' + settings.buttonText + '</span><span class="r"></span></a>')
                        .css({
                            'height': settings.height + 'px',
        ...
        cancel: function(fileID, supressEvent) {
            ...
            this.each(function() {
                ...
                for (var n in swfuploadify.queueData.files) {
                    var queuedFile = swfuploadify.queueData.files[n];
                    if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                        if(swfuploadify.queueData.files[queuedFile.id]) {
    ...
    var handlers = {
        ...
        onDialogOpen: function() {
            ...
            this.queueData.errorMsg = '文件没有添加到成功:';
        ...
        onSelect: function(file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = confirm('文件名为 "' + file.name + '" 的文件已经存在\n是否替换当前已上传的文件?');
                    if (!replaceQueueItem) {
            ...
            if (settings.itemTemplate == false) {
                settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
					<div class="cancel">\
						<a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')" data-id="" class="delete" title="删除">删除</a>\
					</div>\
					<div class="info"><span class="fileName" title="' + _fileName + '">${fileName} (${fileSize})</span><span class="data"></span></div>\
            ...
            if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
                switch (errorCode) {
                    ...
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        ...
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + file.name + '" 不是被支持的文件类型 (' + settings.fileTypeDesc + ').';
                        break;
                }
            }
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 1) {
                            var overwrite = confirm('名为 "' + file.name + '"已经存在.\n你想替换现有文件吗?');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
